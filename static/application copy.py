...
def index():
    ...
    current_position = db.execute("SELECT * FROM buy_sell WHERE buy_sell_id = %s;",(session["user_id"]) )
    ...
    for i in current_position:
        ...
        price_share = db.execute("SELECT * FROM txn_history WHERE txn_id = %s AND symbol = %s;",session["user_id"],i["symbol"])
        total_share = db.execute("SELECT share FROM buy_sell WHERE buy_sell_id = %s AND symbol = %s;", session["user_id"],i["symbol"])[0]["share"]
        sum = 0
...
def buy():
    ...
    if request.method == "POST":
        ...
        balance = db.execute("SELECT cash FROM users WHERE id = %s;", session["user_id"])[0]["cash"]
        ...
        if purchase_cost > balance:
            ...
            update_balance = db.execute("UPDATE users SET cash = %s WHERE id = %s;",new_cash,session["user_id"])

            #(Need to modify this table) Record the txn history to database
            txn_record = db.execute("INSERT INTO txn_history (price,purchase_cost,share,symbol,txn_id,time) VALUES(%s, %s, %s, %s, %s, %s) ;",current_price,purchase_cost,buy_shares,current_symbol,session["user_id"],current_time)
            ...
            try:
                ...
                current_share = db.execute("SELECT share FROM buy_sell WHERE symbol = %s AND buy_sell_id = %s;", current_symbol, session["user_id"])[0]["share"]
            except IndexError:
                #Create database for current stock if not exsited yet
                new_share = db.execute("INSERT INTO buy_sell(buy_sell_id, share, symbol, name) VALUES(%s, %s, %s, %s);",session["user_id"],buy_shares,current_symbol,current_name)
            else:
                #Update share number if current stock already stored in database
                add_share = current_share + int(buy_shares)
                update_share = db.execute("UPDATE buy_sell SET share = %s WHERE symbol = %s AND buy_sell_id = %s;", add_share, current_symbol, session["user_id"])
...
def history():
    ...
    txn_history = db.execute("SELECT * FROM txn_history WHERE txn_id = %s;", session["user_id"])
...
def quote():
    ...
    if request.method == "POST":
        ...
        if quote_detail == None:
            ...
            items=plot(symbol,365)
            return render_template("quote_detail.html", quote_detail=quote_detail,script=items[0],div=items[1])
    else:
...
def register():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO users (username,hash) VALUES(%s, %s)", username,password_hash)
...
def sell():
    ...
    symbols = db.execute("SELECT symbol FROM buy_sell WHERE buy_sell_id = %s;", session["user_id"])
    ...
    if request.method == "POST":
        ...
        current_share = db.execute("SELECT share FROM buy_sell WHERE symbol = %s AND buy_sell_id = %s;", sell_symbol, session["user_id"])[0]["share"]
        ...
        if int(sell_shares) > int(current_share):
            ...
            update_share = db.execute("UPDATE buy_sell SET share = %s WHERE symbol = %s AND buy_sell_id = %s;", min_share, sell_symbol, session["user_id"])

            sell_share = int(sell_shares) * -1
            txn_record = db.execute("INSERT INTO txn_history (price,purchase_cost,share,symbol,txn_id,time) VALUES(%s, %s, %s, %s, %s, %s) ;",sell_price,sell_cost,sell_share,sell_symbol,session["user_id"],current_time)

            # Check if the current stock's number of share is zero, if yes then clear data of this stock data from database
            lastest_share = db.execute("SELECT share FROM buy_sell WHERE symbol = %s AND buy_sell_id = %s;", sell_symbol, session["user_id"])[0]["share"]
            if lastest_share == 0:
                clear = db.execute("DELETE FROM buy_sell WHERE symbol = %s AND buy_sell_id = %s;", sell_symbol, session["user_id"])
...
def add_fund():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = %s WHERE id = %s;",new_deposit,session["user_id"])
        return redirect("/")
    else:
...
def setting():
    if request.method == "POST":
        ...
        old_password_db = db.execute("SELECT hash FROM users WHERE id = %s;",session["user_id"])[0]["hash"]
        ...
        if check_password is True:
            ...
            if new_password == new_password_check:
                ...
                db.execute("UPDATE users SET hash = %s WHERE id = %s;",new_password_hashed,session["user_id"] )
                print("changed")
